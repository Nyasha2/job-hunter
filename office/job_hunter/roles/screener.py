"""Alex — screener with live brief from job_profile.json (assistant updates)."""
...
def _screener_prompt() -> str:
    base = (_ROLES_DIR / "_screener_base.md").read_text(encoding="utf-8")
    brief = _load_brief().render_search_context()
    return f"{base.strip()}\n\n## Current candidate search brief\n\n{brief.strip()}\n"


role = nl_role(_screener_prompt())
